/* Route detail shared component for Pingtung */
...
function appealLabel(score: number) {
  if (score === 5) return "屏東代表地標，值得特地收集";
...
export function RouteDetailPage({ route, hideHeader }: { route: PingtungRoute; hideHeader?: boolean }) {
  const nearby = route.nearbyRoutes
    .map((slug) => pingtungRoutes.find((r) => r.slug === slug))
    .filter(Boolean) as PingtungRoute[];
        ...
        <section className={
          ...
          <div className="mx-auto max-w-3xl">
            <nav className="mb-6 text-xs text-bloom-text-light/60">
              ...
              <a href="/pingtung" className="hover:text-bloom-green">屏東</a>
              <span className="mx-2">/</span>
