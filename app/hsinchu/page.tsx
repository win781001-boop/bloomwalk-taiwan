  ...
  return (
    <div className="flex gap-1" aria-label={`${value}/${max}`}>
      {Array.from({ length: max }, (_, i) => (
        <div key={i} className={`h-1.5 w-5 rounded-full ${i < value ? color : "bg-bloom-green-light/50"}`} />
      ))}
    </div>
  );
}
...
function appealLabel(score: number) {
  if (score === 5) return "新竹縣代表地標，值得特地收集";
      ...
      <main className="flex-1">
        <section className="bg-gradient-to-b from-bloom-sky-light via-bloom-cream to-bloom-cream px-4 pb-16 pt-20 sm:px-6 sm:pb-20 sm:pt-28">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-bloom-green">新竹縣 Hsinchu County</p>
            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-bloom-text sm:text-5xl">新竹縣皮克敏明信片散步路線</h1>
            <p className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-bloom-text-light sm:text-lg">整理新竹縣長期存在、適合皮克敏玩家散步、種花、補步數與收集景點明信片的路線。本縣市路線骨架已建立，內容待後續更新上線。</p>
        ...
        <section className="bg-bloom-cream px-4 pb-24 sm:px-6 sm:pb-32">
          <div className="mx-auto max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-6">
            {hsinchuRoutes.map((route) => (<Card key={route.slug} route={route} />))}
          </div>
        </section>
      </main>
      <footer className="border-t border-bloom-green-light/40 bg-bloom-cream px-4 py-10 sm:px-6">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-4 text-center">
          ...
          <p className="max-w-lg text-xs leading-relaxed text-bloom-text-light/60">
            BloomWalk 是玩家自製的散步路線整理網站，非官方網站，與 Nintendo、Niantic 或 Pikmin Bloom 官方無關。路線內容僅供參考，實際交通、開放時間與場地狀況請以官方公告及現場資訊為準。
          </p>
    ...
    <div className="rounded-2xl border border-bloom-green-light/50 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-md sm:p-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-1 text-xs font-medium text-bloom-gold">{route.postcardAppeal}/5 — {route.postcardAppeal >= 5 ? "新竹縣代表地標，值得特地收集" : route.postcardAppeal >= 4 ? "有特色，適合順路安排" : "實用散步點，明信片吸引力普通"}</p>
          <p className="mt-1 text-xs leading-relaxed text-bloom-text-light/70">{route.postcardReason}</p>
        </div>
        <div className="space-y-3">
          <div>
            <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-bloom-text-light">🚶 散步舒適度</p>
          ...
          <div>
            <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-bloom-text-light">🚌 交通便利度</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {route.tags.map((tag) => (
          <span key={tag} className="inline-block rounded-full border border-bloom-green-light/40 bg-bloom-green-light/30 px-3 py-0.5 text-xs text-bloom-green-dark">{tag}</span>
        ))}
      </div>
      <div className="mt-5">
        <a href={"/hsinchu/" + route.slug} className="inline-flex items-center gap-1.5 text-sm font-medium text-bloom-green transition-all duration-300 hover:gap-2.5 hover:text-bloom-green-dark">
          查看路線
          <svg className="h-3.5 w-3.5 transition-transform duration-300" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
        </a>
      </div>
    </div>
  );
