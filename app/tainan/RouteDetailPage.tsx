/* Route detail shared component for Tainan */
import type { TainanRoute } from "@/lib/tainan-routes";
import { tainanRoutes } from "@/lib/tainan-routes";
...
function appealLabel(score: number) {
  if (score === 5) return "臺南代表地標，值得特地收集";
...
export function RouteDetailPage({ route, hideHeader }: { route: TainanRoute; hideHeader?: boolean }) {
  const nearby = route.nearbyRoutes
    .map((slug) => tainanRoutes.find((r) => r.slug === slug))
    .filter(Boolean) as TainanRoute[];
        ...
        <section className={
          ...
          <div className="mx-auto max-w-3xl">
            <nav className="mb-6 text-xs text-bloom-text-light/60">
              ...
              <a href="/tainan" className="hover:text-bloom-green">臺南</a>
              <span className="mx-2">/</span>
          ...
          <div className="mx-auto max-w-3xl space-y-10">
            ...
            <Section title="🍉 附近休息與補給">
              <p className="mb-3 text-xs leading-relaxed text-bloom-text-light/60">
                以下為周邊補給類型，非特定店家推薦。詳細營業時間與狀況請以現場為準。
              </p>
              <ul className="space-y-2">
