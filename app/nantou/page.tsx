...
function appealLabel(score: number) {
  if (score === 5) return "南投縣代表地標，值得特地收集";
      ...
      <main className="flex-1">
        <section className="bg-gradient-to-b from-bloom-sky-light via-bloom-cream to-bloom-cream px-4 pb-16 pt-20 sm:px-6 sm:pb-20 sm:pt-28">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-bloom-green">南投縣 Nantou</p>
            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-bloom-text sm:text-5xl">南投縣皮克敏明信片散步路線</h1>
            <p className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-bloom-text-light sm:text-lg">整理南投縣長期存在、適合皮克敏玩家散步、種花、補步數與收集景點明信片的路線。本縣市路線骨架已建立，內容待後續更新上線。</p>
        ...
        <section className="bg-bloom-cream px-4 pb-24 sm:px-6 sm:pb-32">
          <div className="mx-auto max-w-3xl space-y-6">
            {nantouRoutes.map((route) => (<Card key={route.slug} route={route} />))}
          </div>
        </section>
      </main>
    ...
    <div className="rounded-2xl border border-bloom-green-light/50 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-md sm:p-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-1 text-xs font-medium text-bloom-gold">{route.postcardAppeal}/5 — {route.postcardAppeal >= 5 ? "南投縣代表地標，值得特地收集" : route.postcardAppeal >= 4 ? "有特色，適合順路安排" : "實用散步點，明信片吸引力普通"}</p>
      ...
      <div className="mt-5">
        <a href={"/nantou/" + route.slug} className="inline-flex items-center gap-1.5 text-sm font-medium text-bloom-green transition-all duration-300 hover:gap-2.5 hover:text-bloom-green-dark">
          查看路線
